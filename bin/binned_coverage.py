...
ref_genome = sys.argv[2]  # path to folder containing mm10 and hg38 chromosome lengths file
working_dir = sys.argv[3]
species = sys.argv[4]

mouse_chroms = get_chrom_lengths(f'{ref_genome}/mm10.chrom.sizes')
human_chroms = get_chrom_lengths(f'{ref_genome}/hg38.chrom.sizes')
...
if species == 'human':
    ...
    bins = create_uniform_bins(mouse_chroms, 1000)
    coverage = map_coverage_to_bins(f"{bigwig_file}", bins)

output = pd.Series(coverage)
output.to_csv(f"{working_dir}/{barcode}_binned_coverage.csv", index=False)
