...
# Sample names are in the format: AAVS_Clone_4_P1_S2_L001_R1_001.fastq.gz
# We want to extract: AAVS_Clone_4_P1, AAVS_Clone_4_P2, etc. (keeping passage info)
...
ls "${FASTQ_DIR}"/${PATTERN}_P*_R1_001.fastq.gz 2>/dev/null | \
    sed 's|.*/||' | \
    sed 's/_S[0-9]*_L[0-9]*_R[0-9]*_001.fastq.gz//' | \
    sort -u > "${OUTPUT_FILE}"
