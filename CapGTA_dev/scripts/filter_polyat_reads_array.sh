...
###########################################################################################################################
# One SLURM array task per BAM: filter to reads with a run of >=10 A or 10 T anywhere in the sequence.
#
# Usage:
#   sbatch --array=1-$(wc -l < bam_list.txt) \
#       CapGTA_dev/scripts/filter_polyat_reads_array.sh <bam_list.txt> <output_dir>
#
# Writes:
#   <output_dir>/<source_basename>.bam
...
python3 "${REPO_ROOT}/CapGTA_dev/scripts/filter_polyat_reads.py" "${SRC_BAM}" "${OUT_BAM}"
