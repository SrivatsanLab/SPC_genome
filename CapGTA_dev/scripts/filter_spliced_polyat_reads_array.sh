...
###########################################################################################################################
# One SLURM array task per BAM: filter to (CIGAR contains N) OR (seq contains >=10 A/T run).
#
# Usage:
#   sbatch --array=1-$(wc -l < bam_list.txt) \
#       CapGTA_dev/scripts/filter_spliced_polyat_reads_array.sh <bam_list.txt> <output_dir>
###########################################################################################################################
...
python3 "${REPO_ROOT}/CapGTA_dev/scripts/filter_spliced_polyat_reads.py" "${SRC_BAM}" "${OUT_BAM}"
